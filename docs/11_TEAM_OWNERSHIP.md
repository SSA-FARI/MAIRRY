...
공통 계약을 작은 PR로 먼저 병합하고 소비자 구현은 해당 계약을 기준으로 진행한다. 작업 브랜치는
`develop`에서 만들고 매일 최소 한 번 최신 `develop`을 반영한다. 배포 시에만 `develop → main`
release PR을 사용하며, 골든 패스가 깨진 상태를 다음 단계로 넘기지 않는다. 상세한 Git 규칙은
