...
async def validation_error_handler(
    ...
    is_contract_payload = (
        ...
        and (
            ...
            or request.url.path.startswith("/api/contracts/")
        )
        and all(error["loc"] and error["loc"][0] == "body" for error in exc.errors())
    )
    response_status = (
        status.HTTP_422_UNPROCESSABLE_CONTENT
        if is_contract_payload
        else status.HTTP_400_BAD_REQUEST
    )
    error_code = (
        ErrorCode.EXTRACTION_VALIDATION_ERROR if is_contract_payload else ErrorCode.VALIDATION_ERROR
    )
    return _response(
        response_status,
        ErrorBody(
